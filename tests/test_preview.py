...
class TestPreview(E2ETestBase):
    ...
    def test_validate_custom_sample_outputs_uses_custom_input_without_sample_only(self) -> None:
        ...
        logs_root = artifact_root / "logs"
        logs_root.mkdir(parents=True, exist_ok=True)
        answer_file = runtime.runtime_blob_store.put_bytes(b"custom-answer\n")
        plan = VerificationTestPlan(
            test_name="001.in",
            source_kind="manual",
            display_source_path="manual_validate.cpp",
        ...
        with patch.object(runtime.judgehost_task_service, "enqueue_task", side_effect=_fake_enqueue_task), patch.object(
            ...
            result = runtime.verification_sanity_service.validate_sample_outputs(
                ...
                accepted_source_file=runtime.runtime_blob_store.put_bytes(b"int main(){return 0;}\n"),
        ...
        first_input = runtime.runtime_blob_store.read(
            PayloadFile.from_payload(first_test["input_file"])
        )
        first_answer = runtime.runtime_blob_store.read(
            PayloadFile.from_payload(first_test["answer_file"])
        )
        second_input = runtime.runtime_blob_store.read(
            PayloadFile.from_payload(second_test["input_file"])
        )
        second_answer = runtime.runtime_blob_store.read(
    ...
    def test_custom_sample_outputs_share_one_accepted_program_across_tests(self) -> None:
        ...
        answer_refs = {
            test_name: runtime.runtime_blob_store.put_bytes(
                f"answer-{test_name}\n".encode("utf-8")
            ).blob_ref
            for test_name in ("001.in", "002.in")
        }
        ...
        with patch.object(
            ...
            result = runtime.verification_sanity_service.validate_sample_outputs(
                ...
                accepted_source_label="solutions/std.cpp",
                accepted_source_name="std.cpp",
                accepted_source_file=runtime.runtime_blob_store.put_bytes(
                    b"int main(){return 0;}\n"
                ),
                run_verification_payload_base=payload_base,
            )
