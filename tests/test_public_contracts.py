...
class TestPublicContracts(unittest.TestCase):
    ...
    def test_workspace_primary_action_keeps_shared_button_alignment(self) -> None:
        ...
        history_source = (ROOT / "app" / "template" / "history.html").read_text(encoding="utf-8-sig")
        ...
        self.assertIn(">Review workspace</a>", base_source)
        self.assertIn(">Backup &amp; Restore</a>", base_source)
        self.assertNotIn("problem_href('export_snapshot')", base_source)
        self.assertNotIn(">Download Snapshot</button>", base_source)
        self.assertNotIn("<h2>Download Snapshot</h2>", export_source)
        self.assertNotIn("<h2>Import Into Workspace</h2>", export_source)
        self.assertIn("problem_href('problem_history_snapshot')", history_source)
        self.assertIn("problem_href('problem_history_import')", history_source)
        self.assertIn(">Download current workspace</button>", history_source)
        self.assertIn("<h2>Import Snapshot</h2>", history_source)
        self.assertIn('class="backup-import-section"', history_source)
