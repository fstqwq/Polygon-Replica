...
Problem readers can read the problem, view and rejudge visible verifications, run a standard full verification against their own workspace, cancel an active verification owned by that workspace, and list or download successful packages. Standard verification freezes the reader's workspace and records private execution evidence; it does not authorize publishing that evidence as shared native-package certification. Problem writers can change source, use Custom Run, create packages, and certify a matching shared package from successful full-verification evidence. Direct problem owners and system administrators can manage the problem and its ACL.
