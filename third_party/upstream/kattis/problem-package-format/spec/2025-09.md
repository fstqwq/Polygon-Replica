...
`data/invalid_input/`  | No        | [Invalid input](#invalid-input)               | Invalid test case input for testing input validation
`data/invalid_output/` | No        | [Invalid output](#invalid-output)             | Invalid test case output for testing output validation
...
[Included files](#included-files), if any, must be copied into the submission directory before building the submission.
...
Limits specify resource constraints for submissions and problem package components.
They are divided into two categories: _submission limits_ and _resource guarantees_.

#### Submission limits

Submission limits define what constitutes an acceptable solution to the problem.
These limits must be enforced exactly: a submission that exceeds any of these limits must be rejected,
and a submission that stays within all limits must not be rejected due to resource usage.

Key                  | Description                                              | Default        | Typical system default
-------------------- | -------------------------------------------------------- | -------------- | ----------------------
`time_multipliers`   | optional map, see [Problem timing](#problem-timing)      | see below      |
`time_limit`         | optional float > 0, see [Problem timing](#problem-timing)| see below      |
...
The `time_limit` specifies the maximum CPU time a submission may use per test case.
Systems are also permitted to enforce a reasonable wall-clock time limit to handle submissions that consume little CPU time (for example, by sleeping or blocking on I/O).

The `memory` limit specifies the maximum memory a submission may use.

The `output` limit specifies the maximum total size of output a submission may produce: 
the sum of standard output, standard error, and (when file writing is allowed) the total size of all files that were created or modified by the submission.

The `code` limit specifies the maximum total size of the submitted files of a submission.

#### Resource guarantees

Resource guarantees specify the minimum resources a judging system must provide for compilation, validation, and visualization.
If a system cannot provide the guaranteed resources, it should reject the problem package at install time.
A system may provide more resources than specified, but must provide at least the guaranteed amount.
...
The `compilation_time` and `compilation_memory` limits apply to compiling submissions.
If a submission exceeds these limits during compilation, the verdict is Compile Error.

The `validation_time`, `validation_memory`, and `validation_output` limits apply to running output validators.
Input validators are run at problem preparation time, not during judging, so these limits do not apply to them.
The `validation_output` limit covers all output validator output, including standard output, standard error, and files written to the feedback directory.
If an output validator exceeds any of these limits, the result is a Judge Error.

For interactive and multi-pass problems, time spent by an output validator waiting for the submission does not count toward `validation_time`.
However, systems are permitted to enforce a reasonable wall-clock time limit on the total interaction.
If this wall-clock limit is exceeded (for example, due to the submission and validator both blocking), the result is a Judge Error.

The `validation_time`, `validation_memory`, and `validation_output` limits also apply to input visualizers and output visualizers.
If a visualizer exceeds any of these limits, there is no visualization produced, but this is not a Judge Error.

If a system accepted a problem package but later cannot provide the guaranteed resources during judging, the result is a Judge Error.
...
The `validation_passes` key specifies the number of passes for multi-pass problems.
See [Multi-pass validation](#multi-pass-validation) for details.

#### Defaults and system limits

For most keys, the system default will be used if nothing is specified.
The "typical system default" values are informational and not guaranteed.

Explicitly specifying a limit is different from accepting the system default:
a specified limit is a hard requirement that the system must honor exactly,
while an omitted limit means any reasonable system default is acceptable.
Only specify limits when the problem requires a specific value.

If a system cannot honor any specified limit (including `time_limit`), it must reject the problem package at install time.
...
It is an error if `time_limit` is not an integer multiple of `time_resolution`.
...
A scalar constant is equivalent to a map containing only the key `value`.
...
Constant sequences are tokens (regex words) of the form {% raw %}`{{name}}`{% endraw %} or {% raw %}`{{name.variant}}`{% endraw %}, 
where `name` is one of the names defined in `constants` and `variant` is one of its variant keys or the special key `value`.
Accessing {% raw %}`{{name}}`{% endraw %} is equivalent to {% raw %}`{{name.value}}`{% endraw %}.
Constants are also available in LaTeX problem statements via the dedicated command `\constant{name}` or `\constant{name.variant}`.
Tools should warn if a variant appears to represent a different value than `value` (best-effort heuristics are sufficient).
...
Any constant sequences referring to undefined constants are not replaced, but tools may warn for them.
...
Sample test cases are shared with solvers as part of the problem statement:

- For problem statements provided in LaTeX or Markdown:
  the statement file must contain only the problem description and input/output specifications and no [sample data](#samples-and-full-feedback).
...
- For problem statements provided as PDFs:
  ...
  To avoid confusing solvers, authors should keep any samples embedded in PDFs consistent with the corresponding files in `data/sample/`.

Test cases with `full_feedback` set to `true` are shared with solvers only after a submission fails on that test case.
This obviously cannot be part of the problem statement itself, but the mechanisms for displaying and downloading these test cases are the same as for samples, so they are described here.
If a `hint` is specified for a `full_feedback` test case, it should also be shown to the solver after failing.
See [Samples and full feedback](#samples-and-full-feedback) for details on `full_feedback` and [Test case configuration](#test-case-configuration) for `hint`.

When a test case is shared with the solver, the following should be displayed and available for download by default (this can be overridden as described below):

  - the given [input](#input),
  - the expected output.

When a test case is shared after the solver failed on the test case, the following should also be displayed and available for download:
...
For interactive and multi-pass problems, displaying two files (input and output) typically does not meaningfully capture how solvers are expected to interact with the problem.
Therefore, it is advised to instead provide samples in the form of a `.interaction` file.
This file contains lines starting with `<` and `>`, containing an interaction log with output from the judge starting with `<` and output from the submission starting with `>`.
For multi-pass problems, passes are separated by a line containing `---` (three dashes).
When a multi-pass problem is not interactive, simply start each pass with lines starting with `<` containing the sample input, followed by lines starting with `>` containing the sample output.

Note that if you are using a `.interaction` file you must not provide a `.in.statement`, `.ans.statement`, or `.out` file,
but you must provide a `.in.download` and a `.ans.download` file.

#### Displaying test cases

By default, when a test case is displayed, the `.in` (input) and `.ans` (output) files are used.
This can be configured by adding `.out`, `.in.statement`, `.ans.statement`, and `.interaction` files.

If there are [input](#input) types other than standard input, these should also be displayed.
There is no way to override these files.

Input display may be overridden, by providing at most one of the following files:

  - `.in.statement`
  - `.interaction`

It is an error to provide more than one of these files for a single test case.

Output display may be overridden, by providing at most one of the following files:
...
If a test case input is overridden for display, it must also be overridden for [download](#downloading-test-cases).
Same for output.

If `.out` is used to override the expected output shown in the statement, the `.out` file must pass the output validator; see [Validation of shared test case files](#validation-of-shared-test-case-files).

#### Downloading test cases

By default, when a test case is available for download, the `.in` (input) and `.ans` (output) files are used.
This can be configured by adding `.out`, `.in.download`, and `.ans.download` files.

If there are [input](#input) types other than standard input, these should also be available for download.
There is no way to override these files.

Input download may be overridden by providing:

  - `.in.download`

Output download may be overridden by providing at most one of the following files:
...
If a test case input is overridden for download, it must also be overridden for [display](#displaying-test-cases).
Same for output.

If you want to make other files available for download, you can use [attachments](#attachments).

#### Validation of shared test case files

For non-interactive and non-multi-pass problems, the `.out` files (if present) must pass the output validator.
For non-interactive and non-multi-pass problems, the `.ans` files must pass the output validator if they are not overridden in any way (that is, if they are shown in the statement).
Other files used for display or download (`.in.statement`, `.ans.statement`, `.in.download`, `.ans.download`, `.interaction`) are not validated.
...
The test data is provided as separate [test cases](#test-cases) in subdirectories of `data/`:
...
Except for in [invalid input](#invalid-input) test cases, if the `.in` file exists, then an associated `.ans` file must exist, while the others are optional.
If the `.in` file does not exist, then the other files must not exist.
Note that a test case must not be named `*/test_group`, since `test_group.yaml` would then be configuration for both the test case and test data group.
The table below summarizes the supported files in a test case:
...
`.out`                          | [Sample data](#sample-data)                         | Example submission output, used instead of `.ans` in statement. Must pass validation
`.interaction`                  | [Sample data](#sample-data)                         | Interaction to be displayed instead of `.in`, and `.ans`
`.in.statement`                 | [Displaying test cases](#displaying-test-cases)     | Used instead of `.in` in the statement
`.ans.statement`                | [Displaying test cases](#displaying-test-cases)     | Used instead of `.ans` in the statement
`.in.download`                  | [Downloading test cases](#downloading-test-cases)   | Used instead of `.in` for downloads
`.ans.download`                 | [Downloading test cases](#downloading-test-cases)   | Used instead of `.ans` for downloads
...
There can only be test data groups _or_ test cases in `data/secret`, never both.
...
This means that there must not be any `test_group.yaml` further down in the directory hierarchy.
...
`full_feedback`           | Boolean                             | `true` in `sample`, otherwise `false`          | See [Samples and full feedback](#samples-and-full-feedback).
...
Note that since `.in` is required for all test cases, if there should be no input on standard input, an empty `.in` must still be provided.
For a test case with a non-empty `.in`, it is piped to the submission as standard input.
...
The `.files` directory, if it exists, contains input files available to the submission via file I/O.
...
possibly overwriting the compiled submission file or [included](#included-files) files in the case of name conflicts.

### Running test cases

For each test case, the submission is executed with:

  - the contents of the `.in` file provided on standard input,
  - the contents of the `.files` directory (if it exists) copied into the submission's working directory,
  - the `args` sequence from the [test case configuration](#test-case-configuration) (if specified) passed as command-line arguments.

For non-interactive problems, the submission's standard output is captured and then given to the [output validator](#output-validator) to check correctness.

For interactive problems, the submission and output validator run concurrently and communicate via pipes; see [Invocation](#invocation-2) in the Output validator section.
For multi-pass problems, the submission is executed multiple times; see [Multi-pass validation](#multi-pass-validation).
...
- `full_feedback` specifies whether solvers should get extended feedback when failing the test case.
...
### Samples and full feedback

The `data/sample` directory contains test cases that are shared with solvers as part of the problem statement.
...
The `data/sample` directory must not contain any test data groups. 
It may be missing or be empty if the problem has no samples.

Test cases that have `full_feedback` set to `true` are test cases that are shared with the solvers only after failing on that test case.
Note that samples have `full_feedback` set to `true` by default.

See [Sample data](#sample-data) in the Problem statements section for details on how test cases are presented to solvers.

### Validation test data

The `data` directory may contain directories with test cases that must be validated, but are not used when judging a submission.
Their goal is to ensure the integrity and quality of the validation programs.
...
`permitted`          | Sequence of strings              | `[AC, WA, TL, RE]`                                                                 | All test cases must have a verdict in this subset of `AC`, `WA`, `TL`, `RE`.
`required`           | Sequence of strings              | `[AC, WA, TL, RE]`                                                                 | At least one test case must have a verdict in this subset of `AC`, `WA`, `TL`, `RE`.
...
wrong_answer:
  permitted: [AC, WA]
  required: [WA]
# All cases AC or TL, at least one TL.
time_limit_exceeded:
  permitted: [AC, TL]
  required: [TL]
# All cases AC or RE, at least one RE.
run_time_error:
  permitted: [AC, RE]
  required: [RE]
...
Any submission that must satisfy a `required: TL` requirement (that is, it must `TL` on at least one test case) is used to provide an _upper bound_ on the time limit.
...
Note that this excludes submissions that for example have `required: [TL, RE]`.

Any submission that is not permitted to get `TL` at all (on some subset of cases) (that is, it must satisfy a `permitted:` rule that does not contain `TL`) is used to provide a _lower bound_ on the time limit.
...
The first is equivalent to a `permitted: [AC, WA, RE]` constraint,
and the second to a `required: [TL]` constraint.
...
- All files from `.files` will be copied into the input validator's working directory for validation, see [Working directory](#working-directory).
...
It is recommended to validate the format and contents of `nextpass.in`.
One way to do this is by performing the validation in the output validator program.
Another way is to let the `output_validator` directory contain both the output validator and a separate program for validating `nextpass.in`. Then, they can be guaranteed to run in the correct order using a [run script](#other-programs).
...
The value of `score_aggregation` determines how the score of `secret` or a test data group is computed; see [Scoring test data groups](#scoring-test-data-groups).  
...
The maximum score of a test case in a group with `min` aggregation is `max_score` for the test data group.
...
Only test cases in test data groups with `sum` or `min` aggregation receive a score. 
