...
def switch_workspace(
    ...
    try:
        ...
        user_id = runtime().workspace_service.known_user_id(active_user)
        if user_id is None:
            ensured = runtime().workspace_service.ensure_user(active_user)
        ...
        if "/" in raw_problem:
            ...
            owned_problem_id = runtime().workspace_service.known_problem_id(owned_problem)
            if owned_problem_id is not None:
                safe_problem = owned_problem
            else:
                global_leaf_matches = runtime().workspace_service.problem_slugs_by_leaf(raw_problem, limit=20)
                accessible_leaf_matches = runtime().workspace_service.accessible_problem_slugs_by_leaf(user_id or 0, raw_problem, limit=20)
        ...
        problem_id = runtime().workspace_service.known_problem_id(safe_problem)
        if problem_id is None:
            runtime().workspace_service.ensure_problem(safe_problem)
            runtime().workspace_service.grant_repo_access(safe_problem, active_user, 'owner')
        else:
        ...
        runtime().workspace_service.ensure_workspace(safe_problem, active_user)
...
def workspace_delete(request: Request, problem: str, user: Annotated[str, Depends(require_session_user)]):
    ...
    try:
        runtime().workspace_service.delete_workspace(problem, user)
