...
from app.services.solution_metadata import normalize_expected_behavior, render_solution_desc
...
class PolygonPackageImportService:
    ...
    def _write_maintained_testlib(self, workspace: Path) -> str:
        ...
        upstream = Path(__file__).resolve().parents[2] / upstream_rel
        target_rel = Path("third_party") / "testlib" / "testlib.h"
        if upstream.exists() and upstream.is_file():
            self._write_bytes(workspace, target_rel, upstream.read_bytes())
            return upstream_rel.as_posix()
        self._write_text(workspace, target_rel, "// place fixed testlib.h copy here\n")
        return ""
