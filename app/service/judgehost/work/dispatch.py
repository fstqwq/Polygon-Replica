...
from app.service.judgehost.work.dispatch_cache import (
    DispatchCacheMixin,
    _DomjudgeCacheEntry,
)
...
class DispatchHandler(DispatchCacheMixin):
    ...
    def complete_task_exposure(
        ...
        *,
        ...
        if not self._s.batch_runtime.task_cases_terminal(task_id):
            return
    ...
    def _case_rows(
        ...
        *,
        ...
        for entry in tests_rows:
            ...
            testcase_signature = hash_of_hashes([testcase_input_hash, testcase_answer_hash])
    ...
    def domjudge_register_host(self, hostname: str) -> list[dict[str, object]]:
        ...
        return [
            {"jobid": job_id, "submitid": str(submit_id)} for job_id, submit_id in release.workdirs
        ]

    def _task_payload(self, task: dict[str, object]) -> tuple[str, str, dict[str, object]]:
        task_id = decode_text(raw=task.get("task_id"))
        if not task_id:
            raise RuntimeError("missing task_id for DOMjudge compatibility")
        run_id = decode_text(raw=task.get("run_id"))
    ...
    def _config_object(self, raw_json: object) -> dict[str, object]:
    ...
    def _precomputed_bundle(
        self, raw_precomputed: dict[str, object] | None
    ) -> _DomjudgePrecomputedBundle | None:
        if raw_precomputed is None:
            return None
        compile_hash = decode_text(lower=True, raw=raw_precomputed.get("compile_hash"))
        full_compile_key = decode_text(lower=True, raw=raw_precomputed.get("compile_key"))
        run_hash = decode_text(lower=True, raw=raw_precomputed.get("run_hash"))
        compare_hash = decode_text(lower=True, raw=raw_precomputed.get("compare_hash"))
        source_hash = decode_text(lower=True, raw=raw_precomputed.get("source_hash"))
        ...
        compile_files = cast(
            list[tuple[str, bytes, bool]] | None, raw_precomputed.get("compile_files")
        )
        run_files = cast(list[tuple[str, bytes, bool]] | None, raw_precomputed.get("run_files"))
        compare_files = cast(
            list[tuple[str, bytes, bool]] | None, raw_precomputed.get("compare_files")
        )
    ...
    def _prepare_payload(
        self, payload: dict[str, object], *, compile_only: bool
    ) -> _DomjudgePreparedPayload:
        ...
        tests_payload = cast(
            list[_DomjudgePreparedTestRow] | None, verification_payload.get("tests")
        )
        ...
        run_cfg_obj = self._config_object(verification_payload.get("run_config_json"))
        ...
        return {
            ...
            "verification_source": decode_text(lower=True, raw=payload.get("verification_source"))
            or self._toolkit.task_kind(payload),
            "expected_behavior": decode_text(lower=True, raw=payload.get("expected_behavior")),
    ...
    def _cache_entry(self, raw_entry: dict[str, object] | None) -> _DomjudgeCacheEntry | None:
    ...
    def _stage_task(self, task: dict[str, object], *, execution_signature: str = "") -> int:
        ...
        if latest_task_row is not None:
            ...
            run_id = decode_text(raw=latest_task_row.get("run_id"), default=run_id)
        ...
        verification_program_id = decode_text(raw=payload.get("verification_program_id"))
        task_kind = self._toolkit.task_kind(payload)
        scope_sequence = self._s.batch_runtime.scope_sequence(verification_id)
        ...
        compile_submission = CompileSubmission(
            compile_key=full_compile_key,
            submit_id=submit_id(full_compile_key),
            source_name=source_name,
            source_file=source_file,
        ...
        return self._s.batch_runtime.create_batch_with_cases(
            task_id=task_id,
            run_id=run_id,
    ...
    def stage_task(self, task: dict[str, object]) -> int:
        _task_id, _run_id, payload = self._task_payload(task)
        execution_signature = decode_text(raw=payload.get("execution_signature"))
        return self._stage_task(task, execution_signature=execution_signature)
    ...
    def activate_task_cases(self, task_id: str) -> bool:
        return self._s.batch_runtime.activate_task_cases(
            task_id,
            now_text=now_iso(),
        )

    def _lease_cases(
        self, batch_id: int, hostname: str, max_batchsize: int
    ) -> list[dict[str, object]]:
        now_text = now_iso()
        ...
        if batch_row["status"] != "open":
            return []
        rows = self._s.batch_runtime.lease_cases(
            int(batch_id),
            hostname=hostname,
            limit=int(cap),
            now_text=now_text,
        ...
        run_id_num = int(script_id(run_hash))
        compare_id = int(script_id(compare_hash))
        compile_submission = self._s.batch_runtime.source_submission(
            str(submit_id(batch_row["compile_key"]))
        )
        if compile_submission is None:
            raise RuntimeError("compile submission disappeared")
        contest_id = batch_row["contest_id"] or "local"
        compile_config_json = batch_row["compile_config_json"]
        run_config_json = batch_row["run_config_json"]
        compare_config_json = batch_row["compare_config_json"]
        ...
        self._s.batch_runtime.record_batch_leased(
            hostname,
            int(batch_id),
        ...
        *,
        ...
        while first_transition or time.monotonic() < deadline:
            ...
            with admission_scope as admission_acquired:
                ...
                batch_row = self._s.batch_runtime.select_ready_batch(safe_host)
                if batch_row is not None:
                    ...
                    refreshed = self._s.batch_runtime.fetch_batch(batch_id)
                    if refreshed is None or refreshed["status"] != "open":
                        continue
                    needs_materialization = (
                        refreshed["materialization_state"] != "ready"
                        and self._s.batch_runtime.batch_case_count(
                            batch_id,
                            status="pending",
                        )
                        > 0
                    )
                    if needs_materialization and not self._materialize_batch(batch_id):
                        return []
            ...
            if not long_poll_used:
                long_poll_used = True
                if self._s.batch_runtime.wait_for_ready_batch(self._s.fetch_long_poll_sec):
                    deadline = time.monotonic() + self._CACHE_PROBE_BUDGET_SEC
                    first_transition = True
                    continue
            admission_scope = (
    ...
    def probe_task_case_cache(self, task_ids: list[str]) -> set[str]:
        ...
        for task_id in ordered_task_ids:
            batch = self._s.batch_runtime.batch_for_task(task_id)
            if batch is None:
                continue
        ...
        return {
            task_id
            for task_id in ordered_task_ids
            if self._s.batch_runtime.task_has_cache_pending_cases(task_id)
        }
