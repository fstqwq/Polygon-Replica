...
class DomjudgeToolkit:
    ...
    def contest_id(raw: object) -> str:
        ...
        if not _CONTEST_ID_RE.fullmatch(token):
            return "local"
        return token
    ...
    def _executable_cache_identity(cls, kind: str, executable_hash: str) -> tuple[str, str]:
        ...
        if (
            safe_kind not in cls._EXECUTABLE_KINDS
            or cls._EXECUTABLE_HASH_RE.fullmatch(safe_hash) is None
        ):
    ...
    def public_compile_specs(self) -> list[dict[str, object]]:
        specs = (
            compile_spec(self._s.config_values, language) for language in ("c", "cpp", "java", "py")
        )
        return [
            {
    ...
    def compile_script(
        ...
        *,
        ...
        language, _exts = self.language_extensions(source_name)
        spec = compile_spec(self._s.config_values, language)
        command = " ".join(shlex.quote(token) for token in (spec.command, *spec.command_arguments))
    ...
    def cpp_executable_build_script(self, source_name: str, *, role: str) -> bytes:
        ...
        safe_source = shlex.quote(decode_basename(raw=source_name, default="interactor.cpp"))
    ...
    def task_kind(
        ...
        *,
        ...
        if source == self._TASK_KIND_GENERATE_INPUT or source.endswith(
            f".{self._TASK_KIND_GENERATE_INPUT}"
        ):
            return self._TASK_KIND_GENERATE_INPUT
        if source == self._TASK_KIND_MAIN_CORRECT or source.endswith(
            f".{self._TASK_KIND_MAIN_CORRECT}"
        ):
    ...
    def execution_signature(self, payload: dict[str, object] | None = None) -> str:
        ...
        verification_source = decode_text(lower=True, raw=payload_obj.get("verification_source"))
        precomputed_raw = payload_obj.get("precomputed")
        if not isinstance(precomputed_raw, dict):
            return ""
        compile_hash = decode_text(lower=True, raw=precomputed_raw.get("compile_hash"))
        run_hash = decode_text(lower=True, raw=precomputed_raw.get("run_hash"))
        compare_hash = decode_text(lower=True, raw=precomputed_raw.get("compare_hash"))
        source_hash = decode_text(lower=True, raw=precomputed_raw.get("source_hash"))
        ...
        signature_payload = {
            ...
            "expected_behavior": decode_text(lower=True, raw=payload_obj.get("expected_behavior")),
            "bypass_case_result_cache": parse_bool(
                payload_obj.get("bypass_case_result_cache"), default=False
            ),
            "source_hash": source_hash,
            "compile_hash": compile_hash,
            "run_hash": run_hash,
    ...
    def config(self) -> dict[str, object]:
        return config_payload(self._s.config_values.snapshot())
    ...
    def list_hosts(self) -> list[dict[str, object]]:
        with self._s.state_lock:
            return hosts_payload(self._s.hosts_state)
