...
def compact_payload_for_retention(payload: object) -> dict[str, object]:
    if not isinstance(payload, dict):
        return {}
    compact = {key: value for key, value in payload.items() if str(key) not in _HEAVY_PAYLOAD_KEYS}
        ...
        if isinstance(tests_obj, list):
            for test_obj in tests_obj:
                if not isinstance(test_obj, dict):
                    continue
                compact_test = {
                    key: test_obj[key] for key in _RETAINED_TEST_KEYS if key in test_obj
                }
                if compact_test:
