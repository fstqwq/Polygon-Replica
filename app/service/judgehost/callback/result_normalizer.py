...
def _program_meta_usage(payload: bytes) -> ExecutionUsage:
    meta = parse_metadata(payload.decode("utf-8", errors="replace"))
...
def normalize_captured_case(
    ...
    *,
    ...
    if metadata_blob:
        program_meta = parse_metadata(metadata_blob.decode("utf-8", errors="replace"))
        cpu_total_sec = parse_nonnegative_float(
            program_meta.get("cpu-time"),
            runtime_sec,
        )
        wall_sec = parse_nonnegative_float(
    ...
    debug_feedback = bounded_feedback_text(
        captured.debug_text,
        limit_bytes=limit_bytes,
    )
    if debug_feedback:
    ...
    if incomplete_metadata_passes:
        metadata_warning = "pass metadata missing for passes " + ", ".join(
    ...
    result = build_case_result(
        ...
        wall_sec=wall_sec,
        memory_kb=memory_kb,
        score_text=captured.score_text,
