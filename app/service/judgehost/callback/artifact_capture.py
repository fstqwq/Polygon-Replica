...
def decode_callback_blob(value: object) -> bytes:
    ...
    if isinstance(value, (bytes, bytearray, memoryview)):
        return bytes(value)
    raise RuntimeError("DOMjudge payload must be base64 text or raw bytes")
...
class CaseArtifactCapture:
        ...
        if capture_expected:
            try:
                ...
                if (
                    ...
                    raise InvalidPassBundle("callback pass does not match final-pass-number")
            except InvalidPassBundle as exc:
                pass_bundle = None
                rejected_bundle = True
                warning = "historical pass artifact capture was incomplete: " + str(exc)
        if pass_bundle is None:
            files["teammessage.txt"] = b"" if rejected_bundle else team_message
            if capture_expected and not warning:
                warning = "historical pass artifact capture was incomplete: " "bundle missing"
        else:
            ...
            for bundled_pass in pass_bundle.passes[:-1]:
                if bundled_pass.capture_status != "complete":
                    reduced.setdefault(bundled_pass.capture_status, []).append(bundled_pass.number)
            if reduced:
                groups = [
                    f"passes {', '.join(str(number) for number in numbers)} " f"{status}"
                    for status, numbers in reduced.items()
                ]
                reduced_warning = "historical pass artifacts were reduced: " + "; ".join(groups)
                warning = f"{warning}; {reduced_warning}" if warning else reduced_warning
    ...
    def capture(
        ...
        payloads = {
            name: self._blob_store.put_bytes(content) for name, content in prepared.files.items()
        }
        artifacts = {
