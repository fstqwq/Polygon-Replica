...
def parse_diagnostic_payload(
    ...
    def append_judgehost_log(text: str) -> None:
        ...
        if not decoded:
            return
        normalized = decoded.replace("\r\n", "\n").replace("\r", "\n")
        raw_lines = [item.strip() for item in normalized.split("\n")]
        ...
        for index, line in enumerate(raw_lines):
            ...
            if any(
                ...
                interesting.extend(
                    item
                    for item in raw_lines[max(0, index - 1) : min(len(raw_lines), index + 2)]
                    if item
                )
        if not interesting:
    ...
    def walk_scalars(value: object, *, key_name: str = "") -> list[str]:
        out: list[str] = []
        key_token = key_name.strip().lower()
        if key_token in handled_keys or key_token == "disabled":
            return out
        if isinstance(value, dict):
            for sub_key, sub_value in value.items():
                out.extend(walk_scalars(sub_value, key_name=str(sub_key).strip()))
                if len(out) >= 32:
                    break
            return out
