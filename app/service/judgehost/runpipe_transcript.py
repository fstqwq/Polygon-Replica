...
_METADATA_RE = re.compile(
    rb"\[(?P<seconds> *[0-9]+\.[0-9]{3})s/(?P<payload_bytes>[0-9]+)\]"
)
_HEADER_LIMIT = 128
    ...
    *,
    ...
    try:
        ...
        while offset < raw_size_bytes:
            ...
            if direction in {b"]", b"["}:
                if payload_bytes != 0:
                    raise _MalformedTranscript(event_offset, "EOF event has a non-zero payload length")
            ...
            shown_length = min(payload_bytes, payload_preview_limit) if event_total < event_limit else 0
