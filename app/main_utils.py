...
def _normalize_workspace_rel_path(raw: str | None) -> str:
...
def _normalize_component_source_path(raw: str | None, folder: str, default_filename: str) -> str:
...
def _normalize_optional_component_source_path(raw: str | None, folder: str, label: str) -> str:
    normalized = _normalize_workspace_rel_path(raw)
    if not normalized:
        return ""
    expected_prefix = f"{folder}/"
...
def _normalize_optional_component_source_path_safe(raw: str | None, folder: str, label: str) -> str:
    try:
        return _normalize_optional_component_source_path(raw, folder, label)
    except ValueError:
        return ""


def _compact_error_text(raw: str | None, *, max_chars: int = 240) -> str:
...
def _sanitize_log_text_for_ui(raw: str, *, path_prefixes: list[str | tuple[str, str]] | None = None) -> str:
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="run-upload-check-") as tmp_name:
            ...
            cleaned = _sanitize_log_text_for_ui(
                raw,
            ...
            return _compact_error_text(preferred or lines[0], max_chars=320)
    except Exception as exc:
        return _compact_error_text(str(exc), max_chars=320) or "compile check failed"
...
def workspace_source_compile_check_error(
    ...
    *,
    ...
    if not safe_path:
        return "compile check failed"
    try:
        source = _safe_workspace_path(workspace, safe_path)
